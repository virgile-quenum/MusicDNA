...
def compute_score(dfm, dfd=None, lib=None, playlists=None):
    ...
    for yr in sorted(dfm['year'].unique()):
        yr_artists = set(dfm.loc[dfm['year'] == yr, 'artistName'])
...
def _dim_bars(s):
    ...
    for name, val, icon, detail in dims:
        pct       = int(val / 20 * 100)
        bar_color = (GREEN if pct >= 80 else
                     VIOLET_LIGHT if pct >= 50 else
                     AMBER if pct >= 30 else "#444")
        parts.append(
...
def render(dfm, dfd=None, lib=None, playlists=None):
    ...
    if total >= 85:
        ...
        verdict = "Expert listener. " + labels[strongest].capitalize() + " is exceptional. " + labels[weakest].capitalize() + " is the only gap."
    elif total >= 55:
        verdict = "Above average. Strong " + labels[strongest] + ". " + labels[weakest].capitalize() + " has room to grow."
    elif total >= 40:
        verdict = "Active listener. Your " + labels[strongest] + " sets you apart."
    else:
        verdict = "Casual listener. Your strongest asset is " + labels[strongest] + "."
