...
def _build_playlist_stats(playlists, dfm):
    """Build enriched stats per playlist crossing with listening history."""
    ...
    # build track→last_played lookup
    last_played_map = (
        dfm.groupby(dfm['trackName'].str.lower().str.strip())['ts']
        .max()
        .to_dict()
    )
    # plays per track
    plays_map = (
        dfm.groupby(dfm['trackName'].str.lower().str.strip())['ms']
        .count()
        .to_dict()
    )
    ...
    for pl in playlists:
        ...
        artist_names  = list({t['artist'] for t in tracks if t['artist']})
...
def render(dfm, playlists):
    ...
    # handle lib passed as dict (legacy)
    ...
    st.markdown(
        ...
        "Average utilisation across all playlists: <b style='color:#ccc;'>"
        + str(avg_util) + "%</b> of added tracks have been played at least once."
        "</div>",
        unsafe_allow_html=True
    )
    ...
    tab1, tab2, tab3, tab4 = st.tabs([
        "Active (" + str(n_active) + ")",
        "Archives & Dormant (" + str(n_archive + n_dormant) + ")",
        "Identity Gap",
        "Merge Candidates",
    ])
    ...
    with tab1:
        ...
        st.caption("Sorted by recent plays — the ones you actually reach for.")
        ...
        if active.empty:
            ...
            for i, (_, row) in enumerate(active.iterrows()):
                last = row['last_played'].strftime('%b %Y') if row['last_played'] else "—"
                with cols[i % 2]:
                    _card(
                        ...
                        + str(int(row['total_tracks'])) + " tracks</span>"
                        "<span style='color:" + GREEN + ";font-size:.78em;font-weight:700;'>"
                        + str(int(row['plays_12m'])) + " plays (12m)</span>"
                        "<span style='color:#555;font-size:.78em;'>"
                        + str(int(row['plays'])) + " total plays</span>"
                        ...
                        + (
                            ...
                            if row['pct_never'] > 30 else ""
                        ),
                        border=GREEN
                    )

    # ── Tab 2: Archives & Dormant ─────────────────────────────────────────────
    with tab2:
        st.markdown("### Playlists you built — and stopped using")

        dead = df[df['status'].isin(['Archive', 'Dormant'])].copy()
        dead['last_played_sort'] = dead['last_played'].fillna(pd.Timestamp('2000-01-01'))
        dead = dead.sort_values('last_played_sort')

        if dead.empty:
            st.success("No dormant or archive playlists found. You're organised.")
        else:
            cols = st.columns(2)
            for i, (_, row) in enumerate(dead.iterrows()):
                ...
                with cols[i % 2]:
                    _card(
                        ...
                        "<div style='font-weight:700;color:#666;font-size:.9em;'>" + str(row['name']) + "</div>"
                        ...
                        + str(int(row['total_tracks'])) + " tracks · last played: " + last
                        "</div>",
                        border=color
                    )
        ...
        if not ghosts.empty:
            ...
            st.caption("These have tracks but zero plays in your history.")
            cols = st.columns(2)
            for i, (_, row) in enumerate(ghosts.iterrows()):
                with cols[i % 2]:
                    _card(
                        "<div style='font-weight:700;color:#444;font-size:.9em;'>"
                        + str(row['name']) + "</div>"
                        "<div style='color:#333;font-size:.76em;margin-top:4px;'>"
                        + str(int(row['total_tracks'])) + " tracks · 0 plays"
                        "</div>",
                        border="#333"
                    )
    ...
    with tab3:
        ...
        st.caption(
            "Playlists with the largest % of tracks you added but never played. "
            "Minimum 5 tracks to qualify."
        )

        gap = (
            df[(df['total_tracks'] >= 5) & (df['pct_never'] > 0)]
            .sort_values('pct_never', ascending=False)
            .head(30)
        )
        ...
        if gap.empty:
            ...
            fig = go.Figure(go.Bar(
                ...
                orientation='h',
                marker_color=[
                    RED if p >= 70 else AMBER if p >= 40 else VIOLET
                    for p in gap['pct_never']
                ],
                text=[str(p) + "% unplayed (" + str(int(n)) + "/" + str(int(t)) + " tracks)"
            ...
            st.markdown(
                "<div class='insight'>These playlists reveal curation intent vs. actual use. "
                "High gap = you built a vision you never lived. "
                "Low gap = you curate what you actually need.</div>",
                unsafe_allow_html=True
            )
    ...
    with tab4:
        ...
        st.caption(
            "Pairs of playlists sharing 40%+ of their artists. "
            "Minimum 3 tracks each. Only active or dormant playlists considered."
        )

        candidates = df[
            (df['status'].isin(['Active', 'Dormant'])) &
            (df['total_tracks'] >= 3)
        ...
        if len(candidates) < 2:
            ...
            for i in range(len(candidates)):
                for j in range(i + 1, len(candidates)):
                    row_a = candidates.iloc[i]
                    row_b = candidates.iloc[j]
                    sim = _jaccard(row_a['artist_set'], row_b['artist_set'])
                    if sim >= 0.4:
                        pairs.append({
                            'Playlist A':    row_a['name'],
                            'Playlist B':    row_b['name'],
                            'Similarity':    round(sim * 100),
                            'Shared artists':len(row_a['artist_set'] & row_b['artist_set']),
                            'A tracks':      int(row_a['total_tracks']),
                            'B tracks':      int(row_b['total_tracks']),
                        })

            if not pairs:
                st.info("No strong overlaps found — your playlists are well differentiated.")
            else:
                pairs_df = pd.DataFrame(pairs).sort_values('Similarity', ascending=False)

                cols = st.columns(1)
                for _, row in pairs_df.iterrows():
                    sim_color = GREEN if row['Similarity'] >= 70 else AMBER
                    _card(
                        "<div style='display:flex;justify-content:space-between;align-items:center;'>"
                        "<div>"
                        "<div style='font-weight:700;color:#fff;font-size:.92em;'>"
                        + str(row['Playlist A']) + " + " + str(row['Playlist B']) + "</div>"
                        "<div style='color:#555;font-size:.78em;margin-top:4px;'>"
                        + str(row['Shared artists']) + " shared artists · "
                        + str(row['A tracks']) + " + " + str(row['B tracks']) + " tracks"
                        "</div>"
                        "</div>"
                        "<span style='color:" + sim_color + ";font-size:1.1em;font-weight:900;"
                        "background:" + sim_color + "22;padding:4px 12px;border-radius:10px;'>"
                        + str(row['Similarity']) + "%</span>"
                        "</div>",
                        border=sim_color
                    )
                ...
                st.markdown(
                    "<div class='insight'>"
                    + str(len(pairs_df)) + " merge candidate" + ("s" if len(pairs_df) > 1 else "") +
                    " found. High similarity doesn't mean identical — check both before merging."
                    "</div>",
                    unsafe_allow_html=True
                )
