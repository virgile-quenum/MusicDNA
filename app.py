...
Flat structure version (no modules/ subfolder)
"""
...
if not st.session_state.data_loaded:
    st.markdown(f"""
    <div style='max-width:580px;margin:80px auto;text-align:center;'>
      <div style='font-size:4em;'>🎵</div>
      <h1 style='font-size:2.8em;font-weight:900;margin:12px 0 4px;'>
        Music<span style='color:{VIOLET_LIGHT};'>DNA</span></h1>
      <p style='color:#444;font-size:.85em;margin-bottom:40px;'>powered by DhalsimStream</p>
      <p style='color:#777;font-size:.95em;line-height:1.9;'>
        Upload your Spotify export and discover who you really are as a listener.<br>
        <b style='color:#aaa;'>Not Wrapped. The real version.</b>
      </p>
      <div style='margin-top:36px;background:#0f0f0f;border:1px solid #1e1e1e;
                  border-radius:12px;padding:22px;text-align:left;'>
        <div style='color:{VIOLET_LIGHT};font-weight:700;margin-bottom:10px;font-size:.82em;
                    text-transform:uppercase;letter-spacing:.08em;'>Get your Spotify data</div>
        <div style='color:#666;font-size:.84em;line-height:2.1;'>
          1. Go to <b style='color:#aaa;'>spotify.com/account/privacy</b><br>
          2. Request → <b style='color:#aaa;'>Extended streaming history</b><br>
          3. Wait up to 30 days for Spotify's email<br>
          4. Download the zip → upload it in the sidebar ←
        </div>
      </div>
    </div>""", unsafe_allow_html=True)
    st.stop()
