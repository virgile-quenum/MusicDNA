...
SECTION_YOUR_STORY = ["Overview", "Who You Are", "Musical Horoscope",
                      "Celebrity Twin", "Taste Drift"]
    ...
    if not st.session_state.data_loaded:
        if is_authenticated():
            st.markdown(
                ...
                "Upload your Extended History zip to unlock all analyses."
                "</div></div>",
                unsafe_allow_html=True
            )
        ...
        for p in nav_pages:
            ...
            if sec != current_section:
                current_section = sec
                st.markdown(
                    "<div style='font-size:.7em;font-weight:700;color:" + color + ";"
                    "text-transform:uppercase;letter-spacing:.06em;"
                    "margin:10px 0 3px;padding-left:2px;'>" + sec + "</div>",
                    unsafe_allow_html=True
                )
            if st.button(p, key="nav_" + p, use_container_width=True):
...
elif "Likes Autopsy"      in page: import likes_autopsy;    likes_autopsy.render(dfm, lib, dfd)
