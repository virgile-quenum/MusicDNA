...
def render(dfm, dfd, kids_on):
    ...
    with col1:
        ...
        if kids_on and not dfd.empty:
            ...
            fig = go.Figure()
            fig.add_bar(x=yearly['year'], y=yearly['hours'], name='Your music',
                        marker_color='#7C3AED')
            fig.add_bar(x=yk['year'], y=yk['hours'], name='Daughters',
                        marker_color='#e74c3c')
        ...
        fig.update_layout(plot_bgcolor='#111',paper_bgcolor='#111',
                          font_color='#888',showlegend=kids_on,
    ...
    with col2:
        ...
        npm = first_year.groupby(first_year).size().reset_index()
        npm.columns = ['year','new_artists']
        fig2 = px.bar(npm, x='year', y='new_artists', color_discrete_sequence=['#A78BFA'])
        fig2.update_layout(plot_bgcolor='#111',paper_bgcolor='#111',
                           font_color='#888', xaxis=dict(gridcolor='#222'),
    ...
    st.markdown(f"""
    <div class='insight'>🎧 <b>{my_h:,.0f}h</b> of your music across {yr_max-yr_min+1} years — roughly <b>{my_h/(yr_max-yr_min+1)/365*60:.0f} min/day</b> on average.</div>
    <div class='insight'>🌍 <b>{my_art:,} unique artists</b> and <b>{my_trk:,} unique tracks</b> — a genuinely exceptional breadth of listening.</div>
    <div class='insight'>⏭️ <b>{my_sk:.1f}% skip rate</b> — you are a committed, intentional listener. Industry average is ~25%.</div>
    """, unsafe_allow_html=True)
    if dau_h > 0:
        pct = dau_h/(my_h+dau_h)*100
        st.markdown(f"<div class='insight'>👶 <b>{dau_h:.0f}h ({pct:.0f}%)</b> of total Spotify usage was daughters content — toggle above to explore.</div>", unsafe_allow_html=True)
